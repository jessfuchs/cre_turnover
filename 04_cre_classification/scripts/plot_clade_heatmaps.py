...
DEFAULT_TREE_FILE = CLASS_DIR / "phylogeny" / "results" / "301Fly_HOG_UCLDtree_40species.nw"
...
# Horizontal structure: tree | species labels | climate | gap | heatmap
TREE_WIDTH = 1.85
HEATMAP_SPECIES_LABEL_WIDTH = 1.08
...
FIG_RIGHT = 0.985
FIG_TOP = 0.855
HEATMAP_BOTTOM_MARGIN = 0.43

TITLE_FONTSIZE = 20

# Main annotation level
BLOCK_LABEL_FONTSIZE = 13.5
TREE_AXIS_FONTSIZE = BLOCK_LABEL_FONTSIZE
CRE_AXIS_FONTSIZE = BLOCK_LABEL_FONTSIZE
LEGEND_TITLE_FONTSIZE = BLOCK_LABEL_FONTSIZE

# Secondary annotation level: 1 pt smaller
TREE_TICK_FONTSIZE = BLOCK_LABEL_FONTSIZE - 1
LEGEND_FONTSIZE = BLOCK_LABEL_FONTSIZE - 1

# Heatmap labels
HEATMAP_CRE_FONTSIZE = 14
HEATMAP_SPECIES_FONTSIZE = 15
...
OUTPUT_DPI = 300
TREE_XLABEL_PAD = 16
TREE_TICK_INTERVAL = 10
HEATMAP_TICK_PAD = 4
HEATMAP_XLABEL_PAD = 22
TIER_DIVIDER_EXTENSION = 0.025
...
CLADES = {
    "rufa_group": {
        "title": "Rufa group",
        "focal": "druf",
        "species": ["druf", "dkik", "dbun", "dbir", "d_serrata"],
    },
    "immigrans_group": {
        "title": "Immigrans group",
        "focal": "dimm",
        "species": ["dimm", "dfor", "dsul", "dnas"],
    },
    "obscura_group": {
        "title": "Subobscura group",
        "focal": "dsub",
        "species": ["dsub", "dbif", "dobs"],
    },
    "azteca_affinis_miranda_group": {
        "title": "Azteca group",
        "focal": "dazt",
        "species": ["dazt", "d_affinis", "dmir", "dper"],
    },
    "teissieri_group": {
        "title": "Teissieri group",
        "focal": "dtei",
        "species": ["dtei", "d_simulans", "d_lutescens", "dsuz"],
    },
    "repleta_group": {
        "title": "Repleta group",
        "focal": "d_repleta",
        "species": ["d_repleta", "d_buzzatii", "d_mojavensis", "d_arizonae"],
    },
}
...
CLIMATE_TO_NUM = {"TROP": 0, "ARID": 1, "TEMP": 2, "BORE": 3}
CLIMATE_CMAP = ListedColormap([CLIMATE_COLORS[zone] for zone in ("TROP", "ARID", "TEMP", "BORE")])
CLIMATE_NORM = BoundaryNorm([-0.5, 0.5, 1.5, 2.5, 3.5], CLIMATE_CMAP.N)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot strict focal-clade CRE-state contrasts with phylogeny and climate annotations."
    )
    parser.add_argument(
        "--matrix",
        type=Path,
...
def require_file(path):
    if not path.is_file():
        raise SystemExit(f"ERROR: required file not found or not a regular file:\n{path}")


def require_columns(df, required, source_name):
    missing = sorted(set(required) - set(df.columns))
    if missing:
        raise SystemExit(f"ERROR: {source_name} is missing required columns:\n" + "\n".join(missing))
...
def short_species_name(slug, manifest_by_slug):
    """Return abbreviated species name; externally generated predictions receive '*' ."""
    ...
    return label + ("*" if row["source"] == "external" else "")
...
def prune_tree_to_species(tree_file, species, slug_to_tree):
    ...
    wanted_tree_names = {slug_to_tree[species_name] for species_name in species}
    available_tree_names = {tip.name for tip in tree.get_terminals()}

    missing = sorted(wanted_tree_names - available_tree_names)
    if missing:
        raise SystemExit("ERROR: focal-clade species missing from phylogenetic tree:\n" + "\n".join(missing))
...
def load_manifest(path):
    ...
    require_columns(manifest, {"slug", "species", "source"}, "combined_manifest.tsv")
    ...
    if manifest["slug"].duplicated().any():
        duplicates = sorted(manifest.loc[manifest["slug"].duplicated(keep=False), "slug"].unique())
        raise SystemExit("ERROR: duplicate species slugs in combined_manifest.tsv:\n" + "\n".join(duplicates))

    manifest["tree_name"] = manifest["species"].map(tree_name_from_species)
    if manifest["tree_name"].duplicated().any():
        duplicates = sorted(
            manifest.loc[manifest["tree_name"].duplicated(keep=False), "tree_name"].unique()
        )
        raise SystemExit(
...
def load_traits(path):
    ...
    require_columns(traits, {"tree_name", "climatic_zone"}, "species_traits.tsv")

    for column in ("tree_name", "climatic_zone"):
        traits[column] = traits[column].astype(str).str.strip()

    if traits["tree_name"].duplicated().any():
        duplicates = sorted(traits.loc[traits["tree_name"].duplicated(keep=False), "tree_name"].unique())
        raise SystemExit("ERROR: duplicate tree_name values in species_traits.tsv:\n" + "\n".join(duplicates))
...
def load_matrix(path):
    ...
    if matrix.index.duplicated().any():
        duplicates = sorted(matrix.index[matrix.index.duplicated()].unique().tolist())
        raise SystemExit("ERROR: duplicated CRE IDs in cre_turnover_matrix.tsv:\n" + "\n".join(duplicates))
    return matrix


def validate_clades(matrix, slug_to_tree):
    all_clade_species = sorted({species for config in CLADES.values() for species in config["species"]})

    missing_from_matrix = [species for species in all_clade_species if species not in matrix.columns]
    ...
    missing_from_manifest = [species for species in all_clade_species if species not in slug_to_tree]
    ...
    for clade_name, config in CLADES.items():
        ...
        if focal not in declared_species:
            raise SystemExit(f"ERROR: focal species {focal!r} is not listed in clade {clade_name!r}.")
        if len(declared_species) != len(set(declared_species)):
            raise SystemExit(f"ERROR: duplicate species in clade definition {clade_name!r}.")
...
def build_clade_data(matrix, tree_file, slug_to_tree, tree_to_slug):
    ...
    for clade_name, config in CLADES.items():
        focal = config["focal"]
        declared_species = config["species"]
        clade_tree = prune_tree_to_species(tree_file, declared_species, slug_to_tree)

        tree_tip_names = [tip.name for tip in clade_tree.get_terminals()]
        missing_reverse_mapping = [name for name in tree_tip_names if name not in tree_to_slug]
        ...
        species_order = [tree_to_slug[tree_name] for tree_name in tree_tip_names]
        comparison_species = [species for species in declared_species if species != focal]
        sub = matrix[declared_species].copy()

        focal_mask = sub.apply(lambda row: is_focal_tier1(row, focal, comparison_species), axis=1)
        focal_ids = sorted(
            sub.index[focal_mask].tolist(),
            key=lambda cre_id: (count_turnover_states(sub.loc[cre_id, declared_species]), cre_id),
        )
        ...
        secondary_ids = [cre_id for cre_id in singleton_info if cre_id not in focal_ids]
        species_rank = {species: index for index, species in enumerate(species_order)}
        ...
        if not cre_order:
            raise SystemExit(f"ERROR: no Tier-1 CREs found for {clade_name}.")
        ...
        for plot_order, cre_id in enumerate(cre_order, start=1):
            discordant_species, discordant_state, consensus_state = singleton_info[cre_id]
            n_turnover = count_turnover_states(sub.loc[cre_id, declared_species])
            summary_rows.append({
                "clade": clade_name,
                "clade_title": config["title"],
                "plot_order": plot_order,
                "cre_id": cre_id,
                "category": "focal_tier1" if cre_id in focal_ids else "secondary_tier1",
                "focal_species": focal,
...
def climate_array(species_order, slug_to_tree, tree_to_climate):
    ...
    for species in species_order:
        ...
        if not climate:
            raise SystemExit(f"ERROR: missing climate annotation for {species}.")
        if climate not in CLIMATE_TO_NUM:
            raise SystemExit(f"ERROR: unknown climatic zone {climate!r} for {species}.")
...
def style_tree_axis(ax_tree, tree, shared_ylim):
    ...
    depths = tree.depths()
    max_tree_depth = max(depths[tip] for tip in tree.get_terminals())
    ax_tree.set_xlim(0, max_tree_depth * TREE_RIGHT_PADDING_FACTOR)

    ax_tree.set_ylabel("")
    ax_tree.set_xlabel("Branch length", fontsize=TREE_AXIS_FONTSIZE, labelpad=TREE_XLABEL_PAD)
    ax_tree.xaxis.set_major_locator(MultipleLocator(TREE_TICK_INTERVAL))
    ax_tree.tick_params(axis="x", labelsize=TREE_TICK_FONTSIZE)
...
def draw_species_labels(ax_labels, species_order, species_labels, focal, shared_ylim):
    ax_labels.set_xlim(0, 1)
    ax_labels.set_ylim(*shared_ylim)

    for row_index, (species, label) in enumerate(zip(species_order, species_labels), start=1):
        ax_labels.text(
...
def draw_heatmap(ax_heat, numeric, n_species, n_focal, n_secondary, shared_ylim):
    ...
    ax_heat.tick_params(axis="x", length=3, pad=HEATMAP_TICK_PAD)
    ax_heat.set_xlabel(
        r"$D.\ melanogaster$ reference CRE",
        fontsize=CRE_AXIS_FONTSIZE,
        labelpad=HEATMAP_XLABEL_PAD,
    )
    ...
    if n_focal > 0 and n_secondary > 0:
        divider_x = n_focal - 0.5
        ax_heat.plot(
            [divider_x, divider_x],
            [-TIER_DIVIDER_EXTENSION, 1 + TIER_DIVIDER_EXTENSION],
...
def add_figure_legends(fig):
    """Add CRE-state and climate-zone legends below the full figure."""
    climate_handles = [
        Patch(facecolor=CLIMATE_COLORS["TROP"], edgecolor="none", label="Tropical"),
        Patch(facecolor=CLIMATE_COLORS["ARID"], edgecolor="none", label="Arid"),
        Patch(facecolor=CLIMATE_COLORS["TEMP"], edgecolor="none", label="Temperate"),
    ...
    fig.add_artist(state_legend)


def plot_clade(clade_name, config, clade_info, tree_file, slug_to_tree, manifest_by_slug,
               tree_to_climate, fig_dir):
    focal = config["focal"]
    declared_species = config["species"]
    ...
    invalid_states = sorted(set(pd.unique(heatmap.values.ravel())) - set(STATE_TO_NUM))
    if invalid_states:
        raise SystemExit(
    ...
    numeric = heatmap.apply(lambda column: column.map(STATE_TO_NUM)).astype(float)
    n_species, n_cre = numeric.shape
    shared_ylim = (n_species + 0.5, 0.5)
    y_edges = np.arange(n_species + 1) + 0.5

    species_labels = [short_species_name(species, manifest_by_slug) for species in species_order]
    climate_numeric = climate_array(species_order, slug_to_tree, tree_to_climate)

    fig = plt.figure(figsize=FIGSIZE)
    grid = fig.add_gridspec(nrows=1, ncols=5, width_ratios=GRID_WIDTH_RATIOS, wspace=GRID_WSPACE)
    ...
    tree = prune_tree_to_species(tree_file, declared_species, slug_to_tree)
    for tip in tree.get_terminals():
        tip.name = ""

    Phylo.draw(tree, axes=ax_tree, do_show=False, show_confidence=False)
    style_tree_axis(ax_tree, tree, shared_ylim)
    draw_species_labels(ax_labels, species_order, species_labels, focal, shared_ylim)
    draw_climate_strip(ax_climate, climate_numeric, y_edges, shared_ylim)
    draw_heatmap(ax_heat, numeric, n_species, n_focal, n_secondary, shared_ylim)

    fig.suptitle(config["title"], fontsize=TITLE_FONTSIZE, fontweight="bold", y=0.985)
    fig.subplots_adjust(left=FIG_LEFT, right=FIG_RIGHT, top=FIG_TOP, bottom=HEATMAP_BOTTOM_MARGIN)
...
def report_clade(config, result, declared_species):
    ...
    print(
        ...
        f"({result['n_focal']} focal Tier 1 + {result['n_secondary']} secondary Tier 1)"
    )
    ...
    for display_index, cre_id in enumerate(selected.index, start=1):
        n_turnover = count_turnover_states(selected.loc[cre_id, declared_species])
        category = "focal" if cre_id in focal_ids else "secondary"
        print(f"  {display_index:>2}. {cre_id:<18} {category:<10} turnover states = {n_turnover}")
...
def report_final(written_pngs, written_pdfs, summary_df, out_tsv):
    ...
    print(f"\nFixed figure dimensions: {FIG_WIDTH} x {FIG_HEIGHT} inches")
    ...
    print(f"Tree x-axis tick interval: {TREE_TICK_INTERVAL}")
    print(f"Tier-divider extension: {TIER_DIVIDER_EXTENSION}")
    ...
    print(f"Climate-to-heatmap gap width ratio: {CLIMATE_HEATMAP_GAP_WIDTH}")
    print("Species labels right-aligned before climate strip: YES")
    print("Species labels italicized: YES")
    print("Tree branch lengths modified: NO")
    print("Panel letters A-F: NO")
    print("Legends in individual figures: YES")
    print("\nCRE ordering:")
    print("  Focal Tier 1 -> Secondary Tier 1")
    print("  within blocks: increasing number of turnover_candidate states")
    print("  Secondary tie-break: discordant species in displayed tree order -> CRE ID")
    print(f"\nWrote plotted-CRE table:\n{out_tsv}")
    print(f"Total clade-specific CRE occurrences: {len(summary_df)}")
...
def main():
    ...
    for path in (matrix_file, tree_file, manifest_file, traits_file):
        require_file(path)
    ...
    manifest_by_slug = manifest.set_index("slug")[["species", "source"]].to_dict(orient="index")
    ...
    clade_data, summary_df = build_clade_data(matrix, tree_file, slug_to_tree, tree_to_slug)
    ...
    for clade_name, config in CLADES.items():
        ...
        report_clade(config, result, config["species"])

    report_final(written_pngs, written_pdfs, summary_df, out_tsv)
